...
impl<'r> Responder<'r, 'static> for Error {
    fn respond_to(self, _: &'r Request<'_>) -> response::Result<'static> {
        let status = match self.error_type {
            ...
            ErrorType::TooManyBotCommands => Status::BadRequest,
            ErrorType::DuplicateCommandName => Status::Conflict,
            ErrorType::UnknownInteraction => Status::NotFound,
            ErrorType::AICompanionBotExists => Status::Conflict,
            ErrorType::AICompanionServerLimit { .. } => Status::Forbidden,
            ErrorType::LicenseVerificationFailed { .. } => Status::Forbidden,
            ErrorType::BotMissingEncryptionKey => Status::BadRequest,
            ErrorType::InvalidPublicKey { .. } => Status::BadRequest,
            ErrorType::EncryptionKeyNotFound => Status::NotFound,
            ErrorType::InvitationExpired => Status::Gone,
            ErrorType::EncryptionError { .. } => Status::InternalServerError,
            ...
            ErrorType::InsufficientTier { .. } => Status::Forbidden,
            ErrorType::ServerMemberLimitReached { .. } => Status::Forbidden,
            ErrorType::ViewerLimitReached { .. } => Status::Forbidden,
            ErrorType::ClipDailyLimitReached { .. } => Status::Forbidden,
            ErrorType::ClipStorageLimitReached { .. } => Status::Forbidden,
            ErrorType::ClipDurationNotAllowed => Status::Forbidden,
            ErrorType::ClippingNotEnabled => Status::Forbidden,
            ErrorType::BitrateExceedsTier { .. } => Status::Forbidden,

            ErrorType::StripeError { .. } => Status::BadGateway,
            ErrorType::AlreadySubscribed => Status::Conflict,
            ErrorType::NoActiveSubscription => Status::NotFound,
        };
