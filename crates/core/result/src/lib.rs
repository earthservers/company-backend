...
pub enum ErrorType {
    ...
    UnknownInteraction,
    /// User already has an AI Companion bot registered
    AICompanionBotExists,
    /// AI Companion bot can only be in 1 server at a time
    AICompanionServerLimit {
        message: String,
    },
    /// License verification failed for AI Companion bot
    LicenseVerificationFailed {
        message: String,
    },
    /// Bot has not registered encryption keys
    BotMissingEncryptionKey,
    /// Invalid PEM public key format
    InvalidPublicKey {
        message: String,
    },
    /// Channel encryption key not found
    EncryptionKeyNotFound,
    /// Encrypted channel invitation expired
    InvitationExpired,
    ...
    /// User needs a higher subscription tier for this action
    InsufficientTier {
        required: String,
        message: String,
    },
    /// Server has reached its member limit (based on owner's tier)
    ServerMemberLimitReached {
        max: usize,
        message: String,
    },
    /// Stream has reached its viewer limit (based on streamer's tier)
    ViewerLimitReached {
        max: usize,
        message: String,
    },
    /// User has reached their daily clip creation limit
    ClipDailyLimitReached {
        max: usize,
        message: String,
    },
    /// User has reached their total clip storage limit
    ClipStorageLimitReached {
        max: usize,
        message: String,
    },
    /// Clip duration not allowed for user's tier
    ClipDurationNotAllowed,
    /// Clipping is not enabled on this stream
    ClippingNotEnabled,
    /// Voice channel bitrate exceeds the server owner's tier limit
    BitrateExceedsTier {
        max: u32,
        message: String,
    },
    ...
    /// User already has an active subscription at this tier or higher
    AlreadySubscribed,
