...
auto_derived_partial!(
    ...
    pub struct Bot {
        ...
        /// Type of bot (regular or ai_companion)
        #[serde(default)]
        pub bot_type: BotType,
        /// License type for AI Companion bots (cloud_basic, cloud_pro, cloud_elite, download)
        #[serde(skip_serializing_if = "Option::is_none")]
        pub license_type: Option<String>,
        /// When the license was last verified (ISO 8601)
        #[serde(skip_serializing_if = "Option::is_none")]
        pub license_verified_at: Option<iso8601_timestamp::Timestamp>,
        /// Which server the AI Companion bot has joined (enforces 1-server limit)
        #[serde(skip_serializing_if = "Option::is_none")]
        pub joined_server_id: Option<String>,

        /// RSA public key in PEM format for E2E encryption
        #[serde(skip_serializing_if = "Option::is_none")]
        pub public_key_pem: Option<String>,
        /// When the public key was registered
        #[serde(skip_serializing_if = "Option::is_none")]
        pub public_key_registered_at: Option<iso8601_timestamp::Timestamp>,
    },
    "PartialBot"
);
...
impl Default for Bot {
    fn default() -> Self {
        Self {
            ...
            joined_server_id: None,
            public_key_pem: None,
            public_key_registered_at: None,
        }
    }
}
...
impl Bot {
        ...
        D: Into<Option<PartialBot>>,
    {
        ...
        // Check AI Companion restrictions before creating
        let partial_data = data.into();
        let is_ai_companion = partial_data
            .as_ref()
            .and_then(|p| p.bot_type.as_ref())
            .map(|t| *t == BotType::AiCompanion)
            .unwrap_or(false);

        if is_ai_companion {
            // Enforce: only 1 AI Companion bot per user
            if let Some(_existing) = db.find_ai_companion_bot_by_owner(&owner.id).await? {
                return Err(create_error!(AICompanionBotExists));
            }
        }
