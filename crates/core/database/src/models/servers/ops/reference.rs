...
impl AbstractServers for ReferenceDb {
    ...
    async fn fetch_discoverable_servers(&self, _limit: i64, _offset: i64) -> Result<Vec<Server>> {
        let servers = self.servers.lock().await;
        Ok(servers
            .values()
    ...
    async fn set_server_featured(&self, _server_id: &str, _featured: bool) -> Result<()> {
        Ok(())
    }
}
