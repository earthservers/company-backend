use crate::{company_result::Result, Channel, FieldsChannel, PartialChannel};
...
pub trait AbstractChannels: Sync + Send {
    ...
    /// Store encryption key for a channel (server-side only, base64-encoded)
    async fn set_channel_encryption_key(&self, channel_id: &str, key_b64: &str) -> Result<()>;

    /// Get channel's encryption key (server-side only)
