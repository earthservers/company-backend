...
impl AbstractChannels for MongoDb {
    ...
    /// Store encryption key for a channel (separate collection)
    async fn set_channel_encryption_key(&self, channel_id: &str, key_b64: &str) -> Result<()> {
        self.col::<Document>("channel_encryption_keys")
            .update_one(
                doc! { "_id": channel_id },
                doc! { "$set": { "key": key_b64 } },
            )
            ...
            .await
            .map(|_| ())
            .map_err(|_| create_database_error!("update_one", "channel_encryption_keys"))
    }

    /// Get channel's encryption key
    async fn get_channel_encryption_key(&self, channel_id: &str) -> Result<String> {
        let doc = self
            .col::<Document>("channel_encryption_keys")
            .find_one(doc! { "_id": channel_id })
            .await
            .map_err(|_| create_database_error!("find_one", "channel_encryption_keys"))?
            .ok_or_else(|| create_error!(EncryptionKeyNotFound))?;

        doc.get_str("key")
            .map(|s| s.to_string())
            .map_err(|_| create_error!(EncryptionKeyNotFound))
    }
}
