...
impl AbstractChannels for ReferenceDb {
    ...
    /// Store encryption key for a channel
    async fn set_channel_encryption_key(&self, channel_id: &str, key_b64: &str) -> Result<()> {
        let mut keys = self.channel_encryption_keys.lock().await;
        keys.insert(channel_id.to_string(), key_b64.to_string());
        Ok(())
    }

    /// Get channel's encryption key
    async fn get_channel_encryption_key(&self, channel_id: &str) -> Result<String> {
        let keys = self.channel_encryption_keys.lock().await;
        keys.get(channel_id)
            .cloned()
