...
use company_models::v0::{self, MessageAuthor};
use company_permissions::OverrideField;
...
auto_derived!(
    ...
    pub enum Channel {
        ...
        Group {
            ...
            /// Moderators can kick members, invite members, and edit group settings
    ...
    pub struct VoiceInformation {
        ...
        /// Maximum audio bitrate in bps (default: 96000)
        #[serde(skip_serializing_if = "Option::is_none")]
...
impl Channel {
    ...
    /// Check whether this channel uses E2E encryption
    pub fn is_encrypted(&self) -> bool {
        match self {
            Channel::TextChannel { encrypted, .. } | Channel::Group { encrypted, .. } => {
                *encrypted
            }
            _ => false,
        }
    ...
    pub fn apply_options(&mut self, partial: PartialChannel) {
        match self {
            ...
            Self::Group {
                ...
                if let Some(v) = partial.moderators {
                    *moderators = v;
                }

                if let Some(v) = partial.encrypted {
                    *encrypted = v;
                }
            }
            Self::TextChannel {
    ...
    pub async fn remove_user_from_group(
        ...
        match &self {
            Channel::Group {
                ...
                if moderators.contains(&user.id) {
                    let updated_mods: Vec<String> = moderators
                        .iter()
                        .filter(|m| *m != &user.id)
                        .cloned()
                        .collect();
