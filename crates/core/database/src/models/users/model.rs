...
use crate::{events::client::EventV1, Database, DecorationEquip, File, RatelimitEvent, AMQP};
...
use company_config::{config, FeaturesLimits};
use company_models::v0::{self, UserBadges, UserFlags};
use company_presence::filter_online;
...
auto_derived_partial!(
    ...
    pub struct User {
        ...
        /// Equipped profile decorations
        #[serde(skip_serializing_if = "Option::is_none")]
        pub active_decorations: Option<Vec<DecorationEquip>>,

        /// Presence indicator style: "monster", "middle-right", or "none"
        #[serde(skip_serializing_if = "Option::is_none")]
        pub presence_style: Option<String>,
    },
    "PartialUser"
);
...
auto_derived!(
    ...
    pub struct UserSubscription {
        ...
        /// Unix timestamp when subscription expires (None = lifetime)
        #[serde(skip_serializing_if = "Option::is_none")]
        pub expires_at: Option<i64>,
        /// Stripe customer ID (for future billing integration)
        #[serde(skip_serializing_if = "Option::is_none")]
        pub stripe_customer_id: Option<String>,
        /// Stripe subscription ID (for future billing integration)
        #[serde(skip_serializing_if = "Option::is_none")]
    ...
    pub enum SubscriptionTier {
        ...
        #[default]
        Free,
        /// Basic tier ($5/mo) - 50 viewers, 5 servers, clipping, 50 MB uploads
        Basic,
        /// Pro tier ($10/mo) - 200 viewers, 10 servers, VOD, 100 MB uploads
        #[serde(alias = "Premium")]
        Pro,
        /// Ultra tier ($20/mo) - unlimited viewers, 20 servers, full VOD, 500 MB uploads
        Ultra,
    }
);
...
impl SubscriptionTier {
    ...
    /// Whether this tier can enable clipping on their streams (as a streamer)
    pub fn can_enable_clipping(&self) -> bool {
        !matches!(self, SubscriptionTier::Free)
    }

    /// Allowed clip durations in seconds for this tier (as a clipper)
    pub fn clip_durations_seconds(&self) -> &[u32] {
        match self {
            SubscriptionTier::Free => &[60],                       // 1 min only (clipping premium streams)
            SubscriptionTier::Basic => &[60, 300],                 // 1, 5 min
            SubscriptionTier::Pro => &[60, 300, 600, 900],         // 1, 5, 10, 15 min
            SubscriptionTier::Ultra => &[60, 300, 600, 900, 3600], // Up to 60 min
        }
    }

    /// Maximum clips per day (None = unlimited)
    pub fn clips_per_day(&self) -> Option<u32> {
        match self {
            SubscriptionTier::Free => Some(3),
            SubscriptionTier::Basic => Some(10),
            SubscriptionTier::Pro => Some(30),
            SubscriptionTier::Ultra => None,
        }
    }

    /// Maximum total clips stored (None = unlimited)
    pub fn max_clips_stored(&self) -> Option<u32> {
        match self {
            SubscriptionTier::Free => Some(5),
            SubscriptionTier::Basic => Some(50),
            SubscriptionTier::Pro => Some(200),
            SubscriptionTier::Ultra => None,
        }
    }

    /// Maximum clip storage in GB (None = unlimited, Ultra has fair use limit)
    pub fn max_clip_storage_gb(&self) -> Option<u32> {
        match self {
            SubscriptionTier::Free => Some(1),
            SubscriptionTier::Basic => Some(2),
            SubscriptionTier::Pro => Some(10),
            SubscriptionTier::Ultra => Some(50), // Fair use limit
        }
    }

    /// Clip retention in days if no views (None = permanent)
    pub fn clip_retention_days(&self) -> Option<u32> {
        match self {
            SubscriptionTier::Free => Some(30),
            SubscriptionTier::Basic => Some(90),
            SubscriptionTier::Pro => Some(365),
            SubscriptionTier::Ultra => None,
        }
    }

    /// Whether this tier can download clips
    pub fn can_download_clips(&self) -> bool {
        matches!(self, SubscriptionTier::Pro | SubscriptionTier::Ultra)
    }

    // -- VOD limits --

    /// Whether VOD recording is enabled for this tier
    pub fn vod_recording_enabled(&self) -> bool {
        matches!(self, SubscriptionTier::Pro | SubscriptionTier::Ultra)
    }

    /// VOD buffer length in minutes (None = disabled or full stream)
    pub fn vod_buffer_minutes(&self) -> Option<u32> {
        match self {
            SubscriptionTier::Free | SubscriptionTier::Basic => None,
            SubscriptionTier::Pro => Some(120),  // 2 hours
            SubscriptionTier::Ultra => None,      // Full stream recording
        }
    }

    /// Whether VOD recordings are stored permanently
    pub fn vod_permanent_storage(&self) -> bool {
        matches!(self, SubscriptionTier::Ultra)
    }

    // -- Emoji limits --

    /// Maximum custom emoji slots for servers owned by this tier
    pub fn max_emoji_slots(&self) -> u32 {
        match self {
    ...
    /// Maximum stream viewers for this tier.
    /// Free tier uses P2P only (10 viewers).
    /// Paid tiers use LiveKit SFU for public streams.
    pub fn stream_viewer_limit(&self) -> usize {
        match self {
            SubscriptionTier::Free => 10,
            SubscriptionTier::Basic => 50,
            SubscriptionTier::Pro => 200,
            SubscriptionTier::Ultra => 0, // 0 = unlimited
        }
    }

    /// Whether this tier gets LiveKit SFU for public streams.
    /// Free tier is P2P only.
    pub fn uses_livekit_for_public_streams(&self) -> bool {
        !matches!(self, SubscriptionTier::Free)
    }

    /// Whether this tier has stream recording enabled.
    pub fn stream_recording_enabled(&self) -> bool {
        matches!(self, SubscriptionTier::Pro | SubscriptionTier::Ultra)
    }

    /// Whether this tier gets priority routing for streams.
    pub fn priority_stream_routing(&self) -> bool {
        matches!(self, SubscriptionTier::Ultra)
    }
}
...
impl Default for User {
    fn default() -> Self {
        Self {
            ...
            active_decorations: Default::default(),
            presence_style: Default::default(),
        }
    }
}
...
impl User {
    ...
    pub async fn limits(&self) -> FeaturesLimits {
        ...
        // Check subscription tier and map to config role
        let tier = self.active_subscription_tier();
        let role_key = match tier {
            SubscriptionTier::Basic => Some("basic"),
            SubscriptionTier::Pro => Some("pro"),
            SubscriptionTier::Ultra => Some("ultra"),
            SubscriptionTier::Free => None,
        };

        if let Some(key) = role_key {
            if let Some(tier_limits) = config.features.limits.roles.get(key) {
                return tier_limits.clone();
            }
            // Fall back to legacy "premium" key for any paid tier without a dedicated config
            if let Some(premium_limits) = config.features.limits.roles.get("premium") {
                return premium_limits.clone();
            }
        }

        // Fall back to new_user vs default based on account age
    ...
    /// Check if this user has any active paid subscription
    pub fn is_premium(&self) -> bool {
        !matches!(self.active_subscription_tier(), SubscriptionTier::Free)
    }

    /// Get the user's current active subscription tier, accounting for expiry
    pub fn active_subscription_tier(&self) -> SubscriptionTier {
        if let Some(subscription) = &self.subscription {
            if !matches!(subscription.tier, SubscriptionTier::Free) {
                // Check expiry
                if let Some(expires_at) = subscription.expires_at {
                    let now = std::time::SystemTime::now()
                        ...
                        .as_secs() as i64;
                    if expires_at <= now {
                        return SubscriptionTier::Free; // Expired
                    }
                }
                return subscription.tier.clone(); // Active or lifetime
            }
        }
        SubscriptionTier::Free
    }

    /// Get the user's current subscription tier (alias for active_subscription_tier)
    pub fn subscription_tier(&self) -> SubscriptionTier {
        self.active_subscription_tier()
    }

    /// Check if this user has the given tier or higher
    pub fn has_tier_or_higher(&self, required: &SubscriptionTier) -> bool {
        let current = self.active_subscription_tier();
        current.tier_rank() >= required.tier_rank()
    }

    /// Get max voice bitrate this user can set on their servers
    pub async fn max_voice_bitrate(&self) -> u32 {
        self.limits().await.max_voice_bitrate
    }

    /// Get max stream viewers for this user
    pub async fn max_stream_viewers(&self) -> usize {
        self.limits().await.max_stream_viewers
    }

    /// Get max server members for servers owned by this user
    pub async fn max_server_members(&self) -> usize {
        self.limits().await.max_server_members
    }
    ...
    /// Check if this user can acquire another server (counts owned servers, not memberships)
    pub async fn can_acquire_server(&self, db: &Database) -> Result<()> {
        let limits = self.limits().await;
        if db.fetch_owned_server_count(&self.id).await? < limits.servers {
            Ok(())
        } else {
            Err(create_error!(TooManyServers {
                max: limits.servers,
                message: format!(
                    "You've reached your server limit of {}. Upgrade your subscription to create more servers.",
                    limits.servers
                )
            }))
        }
    ...
    pub async fn ban(&mut self, db: &Database, reason: Option<String>) -> Result<()> {
        let authifier = db.clone().to_authifier().await;
        let mut account = authifier
            .database
            .find_account(&self.id)
        ...
        self.update(
            db,
            PartialUser {
                flags: Some(UserFlags::Banned as i32),
                ..Default::default()
            },
            vec![FieldsUser::Suspension],
        )
        ...
        // Send ban email if reason provided and SMTP is configured
        if let Some(reason) = reason {
            if let EmailVerificationConfig::Enabled { smtp, .. } =
                authifier.config.email_verification
            {
