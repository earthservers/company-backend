...
impl Member {
    ...
    pub async fn remove(
        ...
        // If the removed member is an AI Companion bot, clear its joined_server_id
        if let Ok(bot) = db.fetch_bot(&self.id.user).await {
            if bot.bot_type == crate::BotType::AiCompanion
                && bot.joined_server_id.as_deref() == Some(&self.id.server)
            {
...
mod tests {
    ...
    use company_models::v0::DataCreateServer;
