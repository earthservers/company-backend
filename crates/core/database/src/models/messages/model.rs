...
use company_result::{ErrorType, Result};
use ulid::Ulid;
...
impl Message {
    ...
    pub async fn create_from_api(
        ...
        let mut message_mentions = if let Some(raw_content) = &data.content {
            company_parser::parse_message(raw_content)
        } else {
            company_parser::MessageResults::default()
        };

        message_mentions.mentions_everyone |= mentions_everyone;
        message_mentions.mentions_online |= mentions_online;

        let company_parser::MessageResults {
            mut user_mentions,
            mut role_mentions,
            ...
            match channel {
                ...
                Channel::TextChannel { ref server, .. }=> {
                    ...
                    if let Ok(valid_members) = valid_members {
                        ...
                        company_config::capture_error(&valid_members.unwrap_err());
    ...
    pub async fn send_without_notifications(
        ...
        if is_dm {
            // Ephemeral DM: relay via broker without persisting to database.
            // The message is delivered in real-time via WebSocket pub/sub.
            // If recipients are offline, it's queued in Redis with TTL.
            EventV1::Message(self.clone().into_model(user, member))
                .p(self.channel.to_string())
                .await;
        } else {
            // Server channel: persist to database and publish event.
