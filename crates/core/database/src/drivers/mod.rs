...
impl Database {
    /// Get the underlying MongoDb handle for direct collection access.
    /// Panics if the database is not MongoDb (analytics requires MongoDB).
    #[cfg(feature = "mongodb")]
    pub fn mongodb(&self) -> &MongoDb {
        match self {
            Database::MongoDb(mongo) => mongo,
            _ => panic!("analytics requires MongoDB backend"),
        }
    }
            ...
            email_verification: if !config.api.smtp.host.is_empty() {
                EmailVerificationConfig::Enabled {
                    smtp: SMTPSettings {
                        ...
                        reply_to: Some(
                            config
                                ...
                                .smtp
                                .reply_to
                                .unwrap_or("admin@earthservers.net".into()),
                        ),
                    ...
                    templates: if config.production {
                        Templates {
                            verify: Template {
                                title: "Verify your Company account.".into(),
                            ...
                            reset: Template {
                                title: "Reset your Company password.".into(),
                                text: include_str!("../../templates/reset.txt").into(),
                                url: format!("{}/login/reset/", config.hosts.app),
                                html: Some(include_str!("../../templates/reset.html").into()),
                            },
                            reset_existing: Template {
                                title: "You already have a Company account, reset your password."
                                    .into(),
