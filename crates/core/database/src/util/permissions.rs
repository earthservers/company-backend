...
impl PermissionQuery for DatabasePermissionQuery<'_> {
    ...
    /// Are we a moderator of this group channel?
    async fn are_we_a_group_moderator(&mut self) -> bool {
        if let Some(channel) = &self.channel {
            match channel {
                Cow::Borrowed(Channel::Group { moderators, .. })
                | Cow::Owned(Channel::Group { moderators, .. }) => {
                    moderators.contains(&self.perspective.id)
                }
                _ => false,
            }
        } else {
