...
use company_config::FeaturesLimits;
use company_models::v0::{self, PartialUserVoiceState, UserVoiceState};
use company_permissions::{calculate_channel_permissions, ChannelPermission, PermissionValue};
...
/// Get all channel IDs that currently have active voice users.
/// Uses the Redis set `active_voice_channels` which is maintained by voice state operations.
pub async fn get_active_voice_channel_ids() -> Result<Vec<String>> {
    get_connection()
        .await?
        .smembers::<_, Vec<String>>("active_voice_channels")
        .await
        .to_internal_error()
...
pub async fn create_voice_state(
    ...
    // Track this channel as having active voice users
    get_connection()
        .await?
        .sadd::<_, _, ()>("active_voice_channels", channel_id)
        .await
        .ok();
...
pub async fn delete_voice_state(
    ...
    // If channel is now empty, remove from active_voice_channels tracking set
    let remaining: u32 = get_connection()
        .await?
        .scard(format!("vc_members:{channel_id}"))
        .await
        .unwrap_or(0);
    ...
    if remaining == 0 {
        get_connection()
            .await?
            .srem::<_, _, ()>("active_voice_channels", channel_id)
            .await
            .ok();
    }
...
pub async fn delete_channel_voice_state(
    ...
    // Channel is being fully cleared, remove from active tracking set
    get_connection()
        .await?
        .srem::<_, _, ()>("active_voice_channels", channel_id)
        .await
...
pub async fn delete_voice_channel(voice_client: &VoiceClient, channel_id: &str, server_id: Option<&str>) -> Result<()> {
    ...
    // Also clean up viewer state
    delete_channel_viewer_state(channel_id).await?;
...
/// Store the connection type (participant or viewer) for a user in a voice channel
pub async fn set_voice_connection_type(channel_id: &str, user_id: &str, conn_type: &str) -> Result<()> {
    get_connection()
        .await?
        .set(format!("vc_connection_type:{user_id}:{channel_id}"), conn_type)
        .await
        .to_internal_error()
}

/// Get the connection type for a user in a voice channel
pub async fn get_voice_connection_type(channel_id: &str, user_id: &str) -> Result<Option<String>> {
    get_connection()
        .await?
        .get(format!("vc_connection_type:{user_id}:{channel_id}"))
        .await
        .to_internal_error()
}

/// Delete the connection type for a user in a voice channel
pub async fn delete_voice_connection_type(channel_id: &str, user_id: &str) -> Result<()> {
    get_connection()
        .await?
        .del(format!("vc_connection_type:{user_id}:{channel_id}"))
        .await
        .to_internal_error()
}

/// Add a user to the viewer set for a channel
pub async fn create_viewer_state(channel_id: &str, user_id: &str) -> Result<()> {
    get_connection()
        .await?
        .sadd(format!("vc_viewers:{channel_id}"), user_id)
        .await
        .to_internal_error()
}

/// Remove a user from the viewer set for a channel
pub async fn delete_viewer_state(channel_id: &str, user_id: &str) -> Result<()> {
    get_connection()
        .await?
        .srem(format!("vc_viewers:{channel_id}"), user_id)
        .await
        .to_internal_error()
}

/// Get the number of viewers for a channel
pub async fn get_viewer_count(channel_id: &str) -> Result<u32> {
    get_connection()
        .await?
        .scard(format!("vc_viewers:{channel_id}"))
        .await
        .to_internal_error()
}

/// Get the list of viewer user IDs for a channel
pub async fn get_viewer_list(channel_id: &str) -> Result<Vec<String>> {
    get_connection()
        .await?
        .smembers::<_, Vec<String>>(format!("vc_viewers:{channel_id}"))
        .await
        .to_internal_error()
}

/// Delete all viewer state for a channel (cleanup)
pub async fn delete_channel_viewer_state(channel_id: &str) -> Result<()> {
    get_connection()
        .await?
        .del(format!("vc_viewers:{channel_id}"))
        .await
        .to_internal_error()
}