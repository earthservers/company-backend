...
use company_config::{config, LiveKitNode};
use company_permissions::{ChannelPermission, PermissionValue};
...
impl VoiceClient {
    ...
    pub async fn from_company_config() -> Self {
    ...
    /// Create a LiveKit token for a viewer (hidden, subscribe-only, cannot publish)
    ...
    pub async fn create_room(&self, node: &str, channel: &Channel) -> Result<Room> {
        ...
        // Extract bitrate from channel's voice configuration (default 96 kbps)
        let _audio_bitrate = channel.voice()
        ...
        room.client
            .create_room(
                ...
                CreateRoomOptions {
                    ...
                    // TODO: Pass audio_bitrate to LiveKit room settings
                    // when the livekit-api crate supports max_audio_bitrate
                    ..Default::default()
                },
            )
            .await
