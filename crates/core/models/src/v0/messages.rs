...
auto_derived_partial!(
    ...
    pub struct Message {
        ...
        ///
        /// https://docs.rs/revolt-models/latest/company_models/v0/enum.MessageFlags.html
        #[cfg_attr(
            feature = "serde",
    ...
    pub async fn from(msg: Message, author: Option<MessageAuthor<'_>>, channel: Channel) -> Self {
        ...
        let icon = if let Some(author) = &author {
            if let Some(avatar) = author.avatar() {
                format!("{}/assets/{}", config.hosts.api, avatar)
            } else {
        ...
        let image = msg.attachments.as_ref().and_then(|attachments| {
            attachments
                .first()
                .map(|v| format!("{}/assets/{}", config.hosts.api, v.id))
        });
