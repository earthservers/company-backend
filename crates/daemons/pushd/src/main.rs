...
use company_config::{config, Settings};
...
async fn main() {
    ...
    company_config::configure!(pushd);
    ...
    if let Some(client) = match &db {
        company_database::Database::Reference(_) => None,
        company_database::Database::MongoDb(mongo) => Some(mongo),
    } {
        authifier =
