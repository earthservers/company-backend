...
use company_models::v0::{MessageFlags, PushNotification};
                ...
                if flags.has(MessageFlags::MentionsEveryone) {
                    ...
                    loop {
                        ...
                        if let Err(err) = self
                            ...
                            .await
                        {
                            company_config::capture_error(&err);
                        }

                        // ignore anyone in this list
                        let online_users = company_presence::filter_online(&userids).await;
                    ...
                    while !exhausted {
                        ...
                        let online = company_presence::filter_online(&viewing_members).await;
                        debug!("online: {:?}", online);
...
impl AsyncConsumer for MassMessageConsumer {
    ...
    async fn consume(
        ...
        if let Err(err) = self
            ...
            .await
        {
            company_config::capture_anyhow(&err);
