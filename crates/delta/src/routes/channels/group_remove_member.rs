...
use company_permissions::ChannelPermission;
...
pub async fn remove_member(
    ...
    {
        let is_owner = &user.id == owner;
        let is_moderator = moderators.contains(&user.id);

        // Only owner or moderators can kick
        ...
        if is_moderator && !is_owner {
            if &member.id == owner || moderators.contains(&member.id) {
                return Err(create_error!(MissingPermission {
                    permission: ChannelPermission::ManageChannel.to_string()
                }));
            }
...
mod test {
    ...
    use company_database::{events::client::EventV1, Channel, RelationshipStatus};
