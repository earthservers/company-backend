...
use company_database::{Interactions, Message, AMQP};
use company_models::v0;
use company_permissions::PermissionQuery;
...
pub async fn message_send(
    ...
    let model_user = user
        .clone()
        .into_known_static(company_presence::is_online(&user.id).await)
        .await;
...
mod test {
    ...
    use company_models::v0::{self, DataCreateServerChannel, MessageFlags};
    use company_permissions::{ChannelPermission, OverrideField};
    use company_result::ErrorType;
