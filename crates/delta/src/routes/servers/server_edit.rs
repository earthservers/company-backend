...
pub async fn edit(
    ...
    // Check we are privileged if changing sensitive fields (flags only)
    if data.flags.is_some() && !user.privileged {
        return Err(create_error!(NotPrivileged));
    }

    // Discoverable can be toggled by server owner (ManageServer already checked above)
    // or by privileged users
