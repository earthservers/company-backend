use company_database::{util::reference::Reference, Database, Message, AMQP};
use company_models::v0::{MessageAuthor, SendableEmbed, Webhook};
...
fn safe_from_str<T: for<'de> Deserialize<'de>>(data: &str) -> Result<T> {
    match serde_json::from_str(data) {
        Ok(output) => Ok(output),
        Err(err) => {
            company_config::capture_internal_error!(err);
