...
use company_config::{config, report_internal_error};
use company_database::{iso8601_timestamp::Timestamp, Database, FileHash, Metadata, User};
use company_files::{
    create_thumbnail, decode_image, fetch_from_s3, upload_to_s3, AUTHENTICATION_TAG_SIZE_BYTES,
};
use company_result::{create_error, Error, Result};
...
async fn fetch_preview(
    ...
    Ok((
        [
            ...
            (
                header::HeaderName::from_static("x-company-deprecation"),
                "File storage is migrating to P2P beacon transfer",
            ),
        ],
        data,
    )
    ...
    retrieve_file_by_hash(&hash).await.map(|data| {
        (
            [
                ...
                (
                    header::HeaderName::from_static("x-company-deprecation"),
                    "File storage is migrating to P2P beacon transfer".to_owned(),
                ),
            ],
            data,
        )
